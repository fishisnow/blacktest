...
def setup_chinese_font():
    ...
    try:
        ...
        if system == "Windows":
            ...
            # Linux系统中文字体
            fonts = ['WenQuanYi Zen Hei', 'WenQuanYi Micro Hei', 'Noto Sans CJK SC', 'Noto Sans CJK']
        ...
        for font in fonts:
            try:
                ...
                if font_path and os.path.exists(font_path) and 'DejaVu' not in font_path:
                    available_font = font
                    print(f"🔍 找到可用中文字体: {font} -> {font_path}")
                    break
            except Exception as e:
                print(f"检测字体 {font} 时出错: {e}")
                continue

        if available_font:
            # 强制设置字体
            plt.rcParams.update({
                'font.sans-serif': [available_font, 'DejaVu Sans'],
...
class ResultAnalyzer:
    ...
    def _analyze_trades(self, trades: List):
        ...
        if trade_pairs:
            ...
            print(
                f"盈利交易: {len(win_trades)}笔, 平均盈利: {np.mean(win_trades):.2f}" if win_trades else "盈利交易: 0笔")
            print(
                f"亏损交易: {len(lose_trades)}笔, 平均亏损: {np.mean(lose_trades):.2f}" if lose_trades else "亏损交易: 0笔")
    ...
    def _create_matplotlib_chart(self, daily_results: List, trades: List) -> str:
        ...
        try:
            ...
            title1, title2 = '账户资产曲线', '每日盈亏'
            ylabel1, ylabel2, xlabel = '资产净值', '盈亏', '日期'
            legend_label = '账户资产'

            # 资产曲线
            ax1.plot(dates, balances, linewidth=2, color='blue', label=legend_label)
